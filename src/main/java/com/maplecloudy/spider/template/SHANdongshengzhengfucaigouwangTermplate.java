 ...
 * Date  2020-11-05
 */
public class SHANdongshengzhengfucaigouwangTermplate extends AbstractTemplate {

  public SHANdongshengzhengfucaigouwangTermplate() throws MalformedURLException {
    // 模板名字，请以网址全程为准
    name = "山东省政府采购网";
    // 爬虫种子页面
    addSeedLink("http://ccgp-shandong.gov.cn/", "首页");
    // 爬虫更新的需要的链接
    addUpdateLink("http://ccgp-shandong.gov.cn/sdgp2017/site/listnew.jsp?grade=city&colcode=2504", "列表第一页");
  ...
  public void genSHOUyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws MalformedURLException {
    ...
    String[][] codes = new String[][]{
        {"province", "2501"}, {"province", "0301"}, {"province", "2102"}, {"province", "0305"}, {"province", "0302"}, {"province", "0306"}, {"province", "2502"}, {"province", "2503"},
        {"city", "2504"}, {"city", "0303"}, {"city", "2106"}, {"city", "0305"}, {"city", "0304"}, {"city", "0306"}, {"city", "2505"}, {"city", "2506"}
    };
    for (int i = 0; i < codes.length; i++) {
      Outlink outlink = new Outlink("http://ccgp-shandong.gov.cn/sdgp2017/site/listnew.jsp?grade=" + codes[i][0] + "&colcode=" + codes[i][1], "");
      outlinks.add(outlink);
    }
  }

  public static String getGrade(String url) {
    return url.split("grade=")[1].split("&")[0];
  }

  public static String getCode(String url) {
    return url.split("colcode=")[1].split("&")[0];
  }
  ...
  public void genLIEbiaodiyiyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws IOException {
    ...
    Elements elements = doc.select("a[href]").select("a[title]").select("a[target]");
    for (Element element : elements) {
      String newUrl = "http://ccgp-shandong.gov.cn/" + element.attr("href");
      Outlink outlink = new Outlink(newUrl, "");
      outlinks.add(outlink);
    }
    Elements select = doc.select("span[id=totalnum]");
    int pageNo = Integer.valueOf(select.text());
    for (int i = 2; i <= pageNo; i++) {
      ...
      BasicNameValuePair[] data = {
          new BasicNameValuePair("grade", getGrade(url.getUrl())),
          new BasicNameValuePair("colcode", getCode(url.getUrl())),
          new BasicNameValuePair("curpage", String.valueOf(i))};
  ...
  public void genXIANGqingyeDatas(Map<String, String> dataMap, Outlink url, Content content, RunMode runMode) throws UnsupportedEncodingException, MalformedURLException {
    ...
    Elements contentElements = doc.select(".n_centent");
...
//    System.out.println(contentHtml);
    ...
    //标题
    dataMap.put("title", doc.select(".title").get(0).text());
    //副标题
...
//    }
    ...
    String projectNum2 = "项目编号[\\s*|:|：]+([^\\s]*)";
    Pattern compileProjectNum2 = Pattern.compile(projectNum2);
    Matcher matcherProjectNum2 = compileProjectNum2.matcher(text);
    if (matcherProjectNum2.find()) {
      dataMap.put("projectnum", matcherProjectNum2.group(1));
    }
    ...
    String projectNum3 = "招标编号[\\s*|:|：]+([^\\s]*)";
    Pattern compileProjectNum3 = Pattern.compile(projectNum3);
    Matcher matcherProjectNum3 = compileProjectNum3.matcher(text);
    if (matcherProjectNum3.find()) {
      dataMap.put("projectnum", matcherProjectNum3.group(1));
    }
    //标的状态
...
//    String budget = "预算金额[\\s*|:|：]+([￥|¥][0-9|\\.*]+\\s*[\u4E00-\u9FA5]*)";
    ...
    String budget8 = "预算金额为[\\s*|:|：]*([0-9]+[\\.|,]*[0-9]*[\\.|,]*[0-9]*\\s*[\u4E00-\u9FA5]*)";
    Pattern compileBudget8 = Pattern.compile(budget8);
    Matcher matcherBudge8 = compileBudget8.matcher(text);
    if (matcherBudge8.find()) {
      dataMap.put("budget", matcherBudge8.group(1));
    }
    ...
    //面包屑索引路径
    dataMap.put("route", doc.select(".sea").get(0).text().split("：")[1]);
    //备用key
...
//    url = "http://ccgp-shandong.gov.cn/sdgp2017/site/listcontnew.jsp?colcode=2504&id=201857373";
    ...
    SHANdongshengzhengfucaigouwangTermplate parse = new SHANdongshengzhengfucaigouwangTermplate();
