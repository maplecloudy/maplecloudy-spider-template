 ...
 * Date  2020-11-07
 */
public class NINGboshiguojizhaobiaowangTermplate extends AbstractTemplate {
    
  public NINGboshiguojizhaobiaowangTermplate() throws MalformedURLException {
    // 模板名字，请以网址全程为准
    name = "宁波市国际招标网";
    // 爬虫种子页面
    addSeedLink("http://www.nbbidding.com/", "首页");
    // 爬虫更新的需要的链接
    addUpdateLink("http://www.nbbidding.com/Home/Notice/news_list?page=1", "列表第一页");
    addUpdateLink("http://www.nbbidding.com/Home/Notice/news_list?page=2", "列表第二页");
    // 网站所有有的链接类型，以及对应的正则
    addDict("首页", "http://www.nbbidding.com/", "GET", "utf-8");
    addDict("列表第一页", "http://www.nbbidding.com/Home/Notice/news_list\\?page=1", "POST", "utf-8");
    addDict("列表页", "http://www.nbbidding.com/Home/Notice/news_list\\?page=\\d+", "GET", "utf-8");
    addDict("详情页", "http://www.nbbidding.com/Home/Notice/news_detail\\?id=\\d+", "GET", "utf-8");
  ...
  public void genSHOUyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws IOException {
    ...
    Outlink outlink = new Outlink("http://www.nbbidding.com/Home/Notice/index?page=1","");
  ...
  public void genLIEbiaodiyiyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws IOException, JSONException {
    ...
    JSONArray rows = json.getJSONObject("data").getJSONArray("list");
    for (int i = 0; i < rows.length(); i++) {
      String id = rows.getJSONObject(i).getString("id");
      String newUrl = "http://www.nbbidding.com/Home/Notice/news_detail?id=" + id;
      Outlink outlink = new Outlink(newUrl, "");
      outlinks.add(outlink);
    }

    int pageNo = json.getJSONObject("data").getJSONObject("page").getInt("totalpage");
  ...
  public void genXIANGqingyeDatas(Map<String,String> dataMap, Outlink url, Content content, RunMode runMode) throws UnsupportedEncodingException, MalformedURLException {
    ...
    Elements contentElements = doc.select(".newsContent");
...
//    System.out.println(contentHtml);
    ...
    //标题
    dataMap.put("title", doc.select(".newsContent>h1").get(0).text());
    //副标题
...
//    String budget = "预算金额[\\s*|:|：]+([￥|¥][0-9|\\.*]+\\s*[\u4E00-\u9FA5]*)";
    ...
    //面包屑索引路径
    dataMap.put("route", doc.select(".site-crumb.w1190").get(0).text());
    //备用key
  ...
  public static void main(String[] args) throws Exception {
    String url;
    url = "http://www.nbbidding.com/";
//    url = "http://www.nbbidding.com/Home/Notice/news_list?page=1";
    ...
    NINGboshiguojizhaobiaowangTermplate parse = new NINGboshiguojizhaobiaowangTermplate();
