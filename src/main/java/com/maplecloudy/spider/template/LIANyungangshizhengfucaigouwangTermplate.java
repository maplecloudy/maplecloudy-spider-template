 ...
 * Date  2020-11-06
 */
public class LIANyungangshizhengfucaigouwangTermplate extends AbstractTemplate {

  public LIANyungangshizhengfucaigouwangTermplate() throws MalformedURLException {
    // 模板名字，请以网址全程为准
    name = "连云港市政府采购网";
    // 爬虫种子页面
    addSeedLink("http://czj.lyg.gov.cn/lygzfcg/", "首页");
    // 爬虫更新的需要的链接
    addUpdateLink("http://czj.lyg.gov.cn/lygzfcg/cgxx/002001/aboutjyxx.html", "列表第一页");
    addUpdateLink("http://czj.lyg.gov.cn/lygzfcg/cgxx/002003/3.htmll", "列表第二页");
    // 网站所有有的链接类型，以及对应的正则
    addDict("首页", "http://czj.lyg.gov.cn/lygzfcg/", "GET", "utf-8");
    addDict("列表第一页", "http://czj.lyg.gov.cn/lygzfcg/cgxx/\\d+/\\w+.html", "GET", "utf-8");
    addDict("列表页", "http://czj.lyg.gov.cn/lygzfcg/cgxx/\\d+/\\d.html", "GET", "utf-8");
    addDict("详情页", "http://czj.lyg.gov.cn/lygzfcg/cgxx/\\d+/\\d+/\\d+/.+.html", "GET", "utf-8");
  ...
  public void genSHOUyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws MalformedURLException {
    ...
    String[] types = new String[]{"002001", "002002", "002003", "002004", "002005", "002006"};
    for (int i = 0; i < types.length; i++) {
      Outlink outlink = new Outlink("http://czj.lyg.gov.cn/lygzfcg/cgxx/00200" + types[i] + "/aboutjyxx.html", "");
  ...
  public void genLIEbiaodiyiyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws UnsupportedEncodingException, MalformedURLException {
    ...
    Elements select = document.select("div.ewb-info-fors>a");
    for (Element element : select) {
      String href = element.attr("href");
      if (href != null) {
        Outlink outlink = new Outlink("http://czj.lyg.gov.cn" + href, "");
        outlinks.add(outlink);
      }
    }

    Element element = document.select(".wb-page-li").get(0);
    Integer page = Integer.valueOf(element.text().split("/")[1]);
    String typeModule = url.getUrl().split("cgxx/")[1].split("/")[0];
    for (int i = 1; i <= page; i++) {
      Outlink outlink = new Outlink("http://czj.lyg.gov.cn/lygzfcg/cgxx/" + typeModule + "/" + i + ".html", "");
      outlinks.add(outlink);
    }
  ...
  //从[详情页]提取链接
  public void genXIANGqingyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws UnsupportedEncodingException, MalformedURLException {
    if (!(runMode == RunMode.BOTH || runMode == RunMode.FETCH))
      return;
...
//    System.out.println(contentHtml);
    ...
    //标题
    dataMap.put("title", doc.select(".news-article-tt").get(0).text());
    //副标题
...
//    String budget = "预算金额[\\s*|:|：]+([￥|¥][0-9|\\.*]+\\s*[\u4E00-\u9FA5]*)";
  ...
  public static void main(String[] args) throws Exception {
    String url;
    url = "http://czj.lyg.gov.cn/lygzfcg/";
    url = "http://czj.lyg.gov.cn/lygzfcg/cgxx/002001/aboutjyxx.html";
    url = "http://czj.lyg.gov.cn/lygzfcg/cgxx/002003/3.html";
        url = "http://czj.lyg.gov.cn/lygzfcg/cgxx/002003/002003001/20200806/59e4c5ea-2701-432d-be7a-0eb3fd6e127c.html";
    ...
    LIANyungangshizhengfucaigouwangTermplate parse = new LIANyungangshizhengfucaigouwangTermplate();
