 ...
 * Date  2020-11-04
 */
public class SHANxishengzhengfucaigouwangTermplate extends AbstractTemplate {

  public SHANxishengzhengfucaigouwangTermplate() throws MalformedURLException {
    // 模板名字，请以网址全程为准
    name = "山西省政府采购网";
    // 爬虫种子页面
    addSeedLink("http://www.ccgp-shanxi.gov.cn/", "首页");
    // 爬虫更新的需要的链接
    addUpdateLink("http://www.ccgp-shanxi.gov.cn/view.php?nav=131", "列表第一页");
    addUpdateLink("http://www.ccgp-shanxi.gov.cn/view.php?app=&type=&nav=131&page=2", "列表第二页");
    // 网站所有有的链接类型，以及对应的正则
    addDict("首页", "http://www.ccgp-shanxi.gov.cn/", "GET", "utf-8");
    addDict("列表第一页", "http://www.ccgp-shanxi.gov.cn/view.php\\?nav=\\d+", "GET", "utf-8");
    addDict("列表页", "http://www.ccgp-shanxi.gov.cn/view.php\\?app=&type=&nav=\\d+&page=\\d+", "GET", "utf-8");
    addDict("详情页", "http://www.ccgp-shanxi.gov.cn/view.php\\?nid=\\d+", "GET", "utf-8");
  ...
  public void genSHOUyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws MalformedURLException {
    if (!(runMode == RunMode.BOTH || runMode == RunMode.FETCH))
     ...
     String[] types = new String[]{"100", "104", "105", "116", "131", "132", "153"};
    for (int i = 0; i < types.length; i++) {
      Outlink outlink = new Outlink("http://www.ccgp-shanxi.gov.cn/view.php?nav=" + types[i],"");
  ...
  public void genLIEbiaodiyiyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws UnsupportedEncodingException, MalformedURLException {
    ...
    String[] split = url.getUrl().split("nav=");
    String[] url_type = split[1].split("&");
    System.out.println(url_type[0]);
    Document doc = Jsoup.parse(html);
    Elements a = doc.select("tbody>tr>td>a");
    int flag = 0;
    for (Element element : a) {
      String href = element.attr("href");
      System.out.println(href);
      if (href != null && flag > 0) {
        Outlink outlink = new Outlink("http://www.ccgp-shanxi.gov.cn/" + href, "");
        outlinks.add(outlink);
      }
      flag++;
    }
    ...
    Element elementPage = doc.selectFirst(".pager");
    String text = elementPage.text();
    String[] split1 = text.split("/");
    String[] splitPage = split1[1].split("页");
    Integer page = Integer.valueOf(splitPage[0]);
    System.out.println(page);
    for (int i = 1; i <= page; i++) {
      Outlink outlink = new Outlink("http://www.ccgp-shanxi.gov.cn/view.php?app=&type=&nav=" + url_type[0] + "&page=" + i, "");
      outlinks.add(outlink);
    }
  }
...
//    System.out.println(contentHtml);
    ...
    //标题
    dataMap.put("title", doc.select("div>h2").get(0).text());
    //副标题
...
//    String budget = "预算金额[\\s*|:|：]+([￥|¥][0-9|\\.*]+\\s*[\u4E00-\u9FA5]*)";
  ...
  public static void main(String[] args) throws Exception {
    ...
    url = "http://www.ccgp-shanxi.gov.cn/view.php?nav=131";
    url = "http://www.ccgp-shanxi.gov.cn/view.php?app=&type=&nav=131&page=2";
    url = "http://www.ccgp-shanxi.gov.cn/view.php?nid=686948";
    ...
    SHANxishengzhengfucaigouwangTermplate parse = new SHANxishengzhengfucaigouwangTermplate();
