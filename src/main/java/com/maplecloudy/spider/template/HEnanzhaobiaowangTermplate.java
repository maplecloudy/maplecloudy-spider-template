 ...
 * Date  2020-11-05
 */
public class HEnanzhaobiaowangTermplate extends AbstractTemplate {

  public HEnanzhaobiaowangTermplate() throws MalformedURLException {
    // 模板名字，请以网址全程为准
    name = "河南招标网";
    // 爬虫种子页面
    addSeedLink("http://www.hnzbw.cn/", "首页");
    // 爬虫更新的需要的链接
    addUpdateLink("http://www.hnzbw.cn/gongcheng/gongcheng_1.asp", "列表第一页");
    addUpdateLink("http://www.hnzbw.cn/zhongbiao/zhongbiao_1.asp?lb=&key=&diqu=&p2=2", "列表第二页");
    // 网站所有有的链接类型，以及对应的正则
    addDict("首页", "http://www.hnzbw.cn/", "GET", "gbk");
    addDict("列表第一页", "http://www.hnzbw.cn/\\w+/\\w+.asp", "GET", "gbk");
    addDict("列表页", "http://www.hnzbw.cn/\\w+/\\w+.asp\\?lb=&key=&diqu=&p2=\\d+", "GET", "gbk");
    addDict("详情页", "http://www.hnzbw.cn/newshow1.asp\\?id=\\d+&l=\\d+", "GET", "gbk");
  ...
  public void genSHOUyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws MalformedURLException {
    ...
    String[] urls = new String[]{
        "http://www.hnzbw.cn/gongcheng/gongcheng_1.asp",
        "http://www.hnzbw.cn/shebei/shebei_1.asp",
        "http://www.hnzbw.cn/fuwu/fuwu_1.asp",
        "http://www.hnzbw.cn/zhongbiao/zhongbiao_1.asp"
    };
    for (int i = 0; i < urls.length; i++) {
      Outlink outlink = new Outlink(urls[i], "");
  ...
  public void genLIEbiaodiyiyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws UnsupportedEncodingException, MalformedURLException {
    ...
    Elements table = doc.select("table").select(".point9").select("tbody>tr");
    ...
    for (int i = 0; i < table.size(); i++) {
      ...
      if (href.contains("id")) {
        String newUrl = "http://www.hnzbcg.com.cn" + href;
    ...
    String text = table.get(21).text();
    int recordNum = Integer.valueOf(text.split("总共")[1].split("条")[0].trim());
    int pageNo = recordNum % 20 == 0 ? recordNum / 20 : recordNum / 20 + 1;
    for (int i = 2; i <= pageNo; i++) {
      Outlink outlink = new Outlink(url.getUrl() + "?lb=&key=&diqu=&p2=" + i, "");
      outlinks.add(outlink);
    }
  ...
  public void genXIANGqingyeDatas(Map<String, String> dataMap, Outlink url, Content content, RunMode runMode) throws UnsupportedEncodingException, MalformedURLException {
    ...
    Elements contentElements = doc.select(".cwzzw");
...
//    String budget = "预算金额[\\s*|:|：]+([￥|¥][0-9|\\.*]+\\s*[\u4E00-\u9FA5]*)";
  ...
  public static void main(String[] args) throws Exception {
    String url;
    url = "http://www.hnzbw.cn/";
    url = "http://www.hnzbw.cn/gongcheng/gongcheng_1.asp";
    url = "http://www.hnzbw.cn/zhongbiao/zhongbiao_1.asp?lb=&key=&diqu=&p2=2";
//    url = "http://www.hnzbw.cn/newshow1.asp?id=2913015&l=1";
    ...
    HEnanzhaobiaowangTermplate parse = new HEnanzhaobiaowangTermplate();
