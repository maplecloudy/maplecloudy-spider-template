 ...
 * Date  2020-11-05
 */
public class ANyangzhaobiaocaigouwangTermplate extends AbstractTemplate {
    
  public ANyangzhaobiaocaigouwangTermplate() throws MalformedURLException {
    // 模板名字，请以网址全程为准
    name = "安阳招标采购网";
    // 爬虫种子页面
    addSeedLink("http://www.ayzbcg.com/", "首页");
    // 爬虫更新的需要的链接
    addUpdateLink("http://www.ayzbcg.com/files/more_sub.asp?key=1288", "列表第一页");
    addUpdateLink("http://www.ayzbcg.com/files/more_sub.asp?key=1285&page=2", "列表第二页");
    // 网站所有有的链接类型，以及对应的正则
    addDict("首页", "http://www.ayzbcg.com/", "GET", "gbk");
    addDict("列表第一页", "http://www.ayzbcg.com/files/more_sub.asp\\?key=\\d+", "GET", "gbk");
    addDict("列表页", "http://www.ayzbcg.com/files/more_sub.asp\\?key=\\d+&page=\\d+", "GET", "gbk");
    addDict("详情页", "http://www.ayzbcg.com/files/showinfo.asp\\?infoid=\\d+", "GET", "gbk");
  ...
  public void genSHOUyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws MalformedURLException {
    ...
    String[] codes = new String[]{"1285", "1286", "1287", "1288"};
    for (int i = 0; i < codes.length; i++) {
      Outlink outlink = new Outlink("http://www.ayzbcg.com/files/more_sub.asp?key=" + codes[i],"");
  ...
  public void genLIEbiaodiyiyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws UnsupportedEncodingException, MalformedURLException {
    ...
    Document doc = Jsoup.parse(new String(content.getContent(), "gbk"));
    Elements select = doc.select("a[href]").select("a[title]").select("a[class]");
    for (Element element : select) {
      String href = element.attr("href").trim();
      if (href.startsWith("../")) {
        href = href.split("\\.\\.")[1];
        String newUrl = "http://www.ayzbcg.com/" + href;
        Outlink outlink = new Outlink(newUrl, "");
        outlinks.add(outlink);
      }
    }
    String text = doc.select("td[valign=bottom]").select("form[name=go2to]").text();
    Integer pageNo = Integer.valueOf(text.split("1/")[1].split("页")[0]);
  ...
  public void genXIANGqingyeDatas(Map<String,String> dataMap, Outlink url, Content content, RunMode runMode) throws MalformedURLException, UnsupportedEncodingException {
    ...
    Elements contentElements = doc.select(".a.cl");
...
//    System.out.println(contentHtml);
    ...
    //标题
    dataMap.put("title", doc.select("head>title").get(0).text());
    //副标题
...
//    String budget = "预算金额[\\s*|:|：]+([￥|¥][0-9|\\.*]+\\s*[\u4E00-\u9FA5]*)";
  ...
  public static void main(String[] args) throws Exception {
    String url;
    url = "http://www.ayzbcg.com/";
    url = "http://www.ayzbcg.com/files/more_sub.asp?key=1288";
    url = "http://www.ayzbcg.com/files/more_sub.asp?key=1285&page=2";
    url = "http://www.ayzbcg.com/files/showinfo.asp?infoid=156";
    ...
    ANyangzhaobiaocaigouwangTermplate parse = new ANyangzhaobiaocaigouwangTermplate();
