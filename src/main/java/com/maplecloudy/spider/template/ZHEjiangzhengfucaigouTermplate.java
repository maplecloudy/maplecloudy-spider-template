 ...
 * Date  2020-11-06
 */
public class ZHEjiangzhengfucaigouTermplate extends AbstractTemplate {

  public ZHEjiangzhengfucaigouTermplate() throws MalformedURLException {
    // 模板名字，请以网址全程为准
    name = "浙江政府采购";
    // 爬虫种子页面
    addSeedLink("http://zfcg.czt.zj.gov.cn/", "首页");
    // 爬虫更新的需要的链接
    addUpdateLink("http://zfcgmanager.czt.zj.gov.cn/cms/api/cors/remote/results?pageSize=15&pageNo=0&sourceAnnouncementType=3011&url=notice", "列表第一页");
    addUpdateLink("http://zfcgmanager.czt.zj.gov.cn/cms/api/cors/remote/results?pageSize=15&pageNo=1&sourceAnnouncementType=3011&url=notice", "列表第二页");
    // 网站所有有的链接类型，以及对应的正则
    addDict("首页", "http://zfcg.czt.zj.gov.cn/", "GET", "utf-8");
    addDict("列表第一页", "http://zfcgmanager.czt.zj.gov.cn/cms/api/cors/remote/results\\?pageSize=\\d+&pageNo=1&sourceAnnouncementType=\\d+&url=notice", "GET", "utf-8");
    addDict("详情页", "http://zfcg.czt.zj.gov.cn/innerUsed_noticeDetails/index.html\\?noticeId=\\d+", "GET", "utf-8");
  ...
  public void genSHOUyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws MalformedURLException {
    ...
    String[] types = new String[]{"10016", "3014", "3013", "1001", "3012%2C1002%2C1003", "3008%2C2001", "3009%2C4004",
        "3001%2C3020", "3002", "3011", "3003", "2002", "3005%2C3017", "3015", "3006", "3018", "3019", "3004%2C4005%2C4006", "3007",
        "3010", "5002", "3016", "6003", "3010"};
    for (int i = 0; i < types.length; i++) {
      Outlink outlink = new Outlink("http://zfcgmanager.czt.zj.gov.cn/cms/api/cors/remote/results?pageSize=15&pageNo=1&sourceAnnouncementType=" + types[i] + "&url=notice", "");
  ...
  public void genLIEbiaodiyiyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws UnsupportedEncodingException, JSONException, MalformedURLException {
    ...
    JSONArray articles = original.getJSONArray("articles");
    for (int i = 0; i < articles.length(); i++) {
      String urlDetail = articles.getJSONObject(i).getString("url");
      Outlink outlink = new Outlink(urlDetail, "");
      outlinks.add(outlink);
    }
    if (url.getUrl().contains("pageNo=1")) {
      int realCount = original.getInt("realCount");
      int pageNo = realCount % 15 == 0 ? realCount / 15 : realCount / 15 + 1;
      String urlTypePage = url.getUrl().split("&sourceAnnouncementType=")[1].split("[&url=]")[0];
      for (int i = 0; i <= pageNo; i++) {
        Outlink outlink = new Outlink("http://zfcgmanager.czt.zj.gov.cn/cms/api/cors/remote/results?pageSize=15&pageNo=" + i + "&sourceAnnouncementType=" + urlTypePage + "&url=notice", "");
        outlinks.add(outlink);
      }
    }
  }
  ...
  public void genXIANGqingyeDatas(Map<String, String> dataMap, Outlink url, Content content, RunMode runMode) throws MalformedURLException, UnsupportedEncodingException {
    ...
    Elements contentElements = doc.select(".ewb-main");
...
//    System.out.println(contentHtml);
    ...
    //标题
    dataMap.put("title", doc.select(".header>h1").get(0).text());
    //副标题
...
//    String budget = "预算金额[\\s*|:|：]+([￥|¥][0-9|\\.*]+\\s*[\u4E00-\u9FA5]*)";
    ...
    //面包屑索引路径
    dataMap.put("route", doc.select(".ewb-location").get(0).text().split("：")[1]);
    //备用key
  ...
  public static void main(String[] args) throws Exception {
    String url;
    url = "http://zfcg.czt.zj.gov.cn/";
    url = "http://zfcgmanager.czt.zj.gov.cn/cms/api/cors/remote/results?pageSize=15&pageNo=1&sourceAnnouncementType=4007&url=notice";
    url = "http://zfcgmanager.czt.zj.gov.cn/cms/api/cors/remote/results?pageSize=15&pageNo=1&sourceAnnouncementType=3011&url=notice";
        url = "http://zfcg.czt.zj.gov.cn/innerUsed_noticeDetails/index.html?noticeId=7033833";
    ...
    ZHEjiangzhengfucaigouTermplate parse = new ZHEjiangzhengfucaigouTermplate();
