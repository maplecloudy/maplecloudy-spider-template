 ...
 * Date  2020-11-05
 */
public class JIANGsushengzhengfucaigouwangTermplate extends AbstractTemplate {

  public JIANGsushengzhengfucaigouwangTermplate() throws MalformedURLException {
    // 模板名字，请以网址全程为准
    name = "江苏省政府采购网";
    // 爬虫种子页面
    addSeedLink("http://www.ccgp-jiangsu.gov.cn/", "首页");
    // 爬虫更新的需要的链接
    addUpdateLink("http://www.ccgp-jiangsu.gov.cn/ggxx/zgysgg/index_1.html", "列表第一页");
    addUpdateLink("http://www.ccgp-jiangsu.gov.cn/ggxx/zgysgg/index_2.html", "列表第二页");
    // 网站所有有的链接类型，以及对应的正则
    addDict("首页", "http://www.ccgp-jiangsu.gov.cn/", "GET", "utf-8");
    addDict("列表第一页", "http://www.ccgp-jiangsu.gov.cn/\\w+/\\w+/", "GET", "utf-8");
    addDict("列表页", "http://www.ccgp-jiangsu.gov.cn/\\w+/\\w+/index_\\d+.html", "GET", "utf-8");
    addDict("详情页", "http://www.ccgp-jiangsu.gov.cn/\\w+/\\w+/\\w+/\\w+/\\w+_\\w+.html", "GET", "utf-8");
  ...
  public void genSHOUyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws MalformedURLException {
    ...
    String[] types = new String[]{"zgysgg/", "gkzbgg/", "yqzbgg/", "jztbgg/", "jzqsgg/", "dylygg/", "xjgg/", "zbgg/", "cgcjgg/", "zzgg/", "cggzgg/", "qtgg/", "htgg_1/"};
    for (int i = 0; i < types.length; i++) {
      Outlink outlink = new Outlink("http://www.ccgp-jiangsu.gov.cn/ggxx/" + types[i], "");
  ...
  public void genLIEbiaodiyiyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws UnsupportedEncodingException, MalformedURLException {
    ...
    Elements select = document.select(".list_list>ul>li>a");
    for (Element element : select) {
      ...
      if (href != null && !href.contains("../../")) {
        ...
        Outlink outlink = new Outlink("http://www.ccgp-jiangsu.gov.cn" + href2, "");
        outlinks.add(outlink);
      }
    }
    String typeModule = url.getUrl().split("cn/")[1];
    Integer page = Integer.valueOf(document.html().split(" createPageHTML\\(")[2].split(",")[0]);
    System.out.println(page);
    for (int i = 1; i <= page; i++) {
      Outlink outlink = new Outlink("http://www.ccgp-jiangsu.gov.cn/" + typeModule + "index_" + i + ".html", "");
      outlinks.add(outlink);
    }
  }
  ...
  public void genLIEbiaoyeLinks(List<Outlink> outlinks, Outlink url, Content content, RunMode runMode) throws UnsupportedEncodingException, MalformedURLException {
    ...
    for (Element element : select) {
      ...
      if (href != null && !href.contains("../../")) {
        String href1 = href.substring(1);
        Outlink outlink = new Outlink(url.getUrl().split("index")[0] + href1, "");
        outlinks.add(outlink);
      } else {
        String href2 = href.substring(4);
        Outlink outlink = new Outlink("http://www.ccgp-jiangsu.gov.cn" + href2, "");
...
//    System.out.println(contentHtml);
    ...
    //标题
    dataMap.put("title", doc.select(".dtit>h1").get(0).text());
    //副标题
...
//    String budget = "预算金额[\\s*|:|：]+([￥|¥][0-9|\\.*]+\\s*[\u4E00-\u9FA5]*)";
    ...
    //面包屑索引路径
    dataMap.put("route", doc.select(".local").get(0).text().split("：")[1]);
    //备用key
...
//        url = "http://www.ccgp-jiangsu.gov.cn/ggxx/zgysgg/shengji/202007/t20200730_701153.html";
    ...
    JIANGsushengzhengfucaigouwangTermplate parse = new JIANGsushengzhengfucaigouwangTermplate();
